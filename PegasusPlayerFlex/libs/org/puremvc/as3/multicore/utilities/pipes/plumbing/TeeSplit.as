...
package org.puremvc.as3.multicore.utilities.pipes.plumbing
{
	...
	import org.puremvc.as3.multicore.utilities.pipes.events.PipeEvent;
	 ...
	 */
	public class TeeSplit extends EventDispatcher implements IPipeFitting
	{
		...
		{
			outputs.push(output);
			output.addEventListener(PipeEvent.DISCONNECT_FITTING, handleFittingDisconnect)
			return true;
		}
		
		/**
		 * Respond to a fitting that has disconnected and clean up. 
		 * @param event
		 */		
		private function handleFittingDisconnect(event:PipeEvent):void
		{
			event.target.removeEventListener(PipeEvent.DISCONNECT_FITTING, handleFittingDisconnect)
			this.disconnectFitting(event.target as IPipeFitting);
		}
